...
const renderMovies = async function () {
  let count = 0;
  let promises = movies.map(async (data) => {
    let err,value;
    const movie = new Movie(data);
    [err, value] = await to(movie.save());
    if(!err)
      count = count + 1;
    return count;
  });
...
module.exports.renderMovies = renderMovies;
const movies = [
  {
    title: 'La La Land',
    poster: 'https://i.imgur.com/po7UezG.jpg',
    genre: 'Drama/Romance',
    days: ['Today', 'Tomorrow', moment().add(2, 'days').format('ddd, MMM D')],
    times: ['9:00 AM', '11:10 AM', '12:00 PM', '1:50 PM', '4:30 PM', '6:00 PM', '7:10 PM', '9:45 PM']
  },
  {
    title: 'Paterson',
